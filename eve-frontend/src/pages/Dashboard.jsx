...
function Dashboard() {
  ...
  const handleRenameSubmit = async (fileId) => {
    ...
    try {
      const response = await fetch(`https://localhost:7213/api/Excel?id=${fileId}&fileName=${newFileName}`, {
        method: 'PUT',
      });
  ...
  const handleDeleteFiles = async () => {
    ...
    try {
      for (const fileId of selectedFiles) {
        const response = await fetch(`https://localhost:7213/api/Excel?id=${fileId}`, {
          method: 'DELETE',
        });
